...
class ReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        // --- TAB 3: Realtime
        $in  = InTransaction::with('item.category')->get()->map(function ($m) {
            $m->type = 'in';
            return $m;
        });
        $out = OutTransaction::with('item.category')->get()->map(function ($m) {
            $m->type = 'out';
            return $m;
        });
    ...
    public function exportPDF(Request $request)
    {
        ...
        if ($request->filled('kode_grup')) $query->where('kode_grup', $request->kode_grup);
    ...
    {
        return Excel::download(new CategoryReportExport, 'Laporan_Kategori_AMW_' . now()->format('Ymd_His') . '.xlsx');
    }
    ...
    private function getRealtimeDataWithFilters(Request $request): array
    {
        ...
        if ($request->filled('type'))       $transactions = $transactions->where('type', $request->type);
        if ($request->filled('category_id')) {
            $catId = (int)$request->category_id;
            $transactions = $transactions->filter(fn($t) => optional(optional($t->item)->category)->id === $catId);
        }
    ...
    public function realtimeData(Request $request)
    {
        try {
            ...
            if ($request->filled('start_date')) $q->whereDate('x.date', '>=', $request->start_date);
            if ($request->filled('end_date'))   $q->whereDate('x.date', '<=', $request->end_date);
            if ($request->filled('type'))       $q->where('x.type', $request->type);
            if ($request->filled('category_id')) $q->where('x.category_id', (int) $request->category_id);

            // 🔹 Kirim ke DataTables
            return DataTables::of($q->orderByDesc('x.date'))
                ->addIndexColumn()
                ->editColumn('date', fn($r) => $r->date ? \Carbon\Carbon::parse($r->date)->format('d M Y') : '-')
                ->editColumn('type', fn($r) => $r->type === 'in'
                    ? '<span class="badge bg-success">Masuk</span>'
                    : '<span class="badge bg-danger">Keluar</span>')
                ->editColumn('item_code', fn($r) => $r->item_code
    ...
    public function distribusiData(Request $request)
    {
        ...
        if ($request->filled('date')) $q->whereDate('ot.date', $request->date);
        if ($request->filled('kode_grup')) $q->where('ot.kode_grup', $request->kode_grup);

        return DataTables::of($q)
            ->addIndexColumn()
            ->editColumn('date', fn($r) => $r->date ? \Carbon\Carbon::parse($r->date)->format('d M Y') : '-')
            ->editColumn('qty', fn($r) => '<strong class="text-success">' . $r->qty . ' ' . e($r->unit) . '</strong>')
            ->rawColumns(['qty'])
            ->toJson();
    }
}
