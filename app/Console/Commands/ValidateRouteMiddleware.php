...
class ValidateRouteMiddleware extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($routes as $route) {
            ...
            if (!str_starts_with($uri, 'sanctum') && !str_starts_with($uri, 'login') && !str_starts_with($uri, 'register')) {
                if (
        ...
        if (empty($invalid)) {
            $this->newLine();
            $this->line("✅ <fg=green>Semua route sudah memiliki middleware yang sesuai (auth & role).</>");
        } else {
            $this->newLine();
            $this->line("⚠️ <fg=yellow>Ditemukan route yang belum memiliki middleware lengkap:</>\n");
            $this->table(
                ['URI', 'Nama Route', 'Middleware Saat Ini'],
                $invalid
            );
        }
        ...
        $this->table(
            ...
            collect(Route::getRoutes())->map(fn($r) => [
                'URI' => $r->uri(),
